...
export class BrowserControlMCPServer {
   ...
   * 发送命令到浏览器并等待结果
   */
  private async sendToBrowser(
    command: BrowserCommandMessage
  ): Promise<unknown> {
      ...
      return {
        tools: [
          ...
          },
          {
            ...
            description:
              "获取页面元素结构（Accessibility Tree）。返回页面上的可交互元素列表，每个元素都有唯一的 ref ID",
            inputSchema: {
              type: "object" as const,
              properties: {
                filter: {
                  ...
                  description:
                    "过滤模式：all=所有元素，interactive=只显示可交互元素（默认）",
                },
              },
            },
          },
            ...
            inputSchema: {
              ...
              properties: {
                key: {
                  type: "string",
                  description:
                    "按键名称，如 Enter, Tab, Backspace, ctrl+a, ctrl+shift+t",
                },
                repeat: {
                  type: "number",
  ...
  private async handleScreenshot(
    ...
    const command = createBrowserCommand({
      name: "screenshot",
      params: { tabId: args?.tabId as number | undefined },
    });

    const result = (await this.sendToBrowser(command)) as {
      base64: string;
      width: number;
      height: number;
    };
  ...
  private async handleClick(
    ...
    const command = createBrowserCommand({
      ...
      params: {
        ...
        clickCount: args.clickCount as 1 | 2 | 3 | undefined,
      },
    });
  ...
  private async handleReadPage(
    ...
    const command = createBrowserCommand({
      name: "read_page",
      params: {
        filter: (args?.filter as "all" | "interactive" | undefined) ?? "interactive",
      },
    });

    const result = (await this.sendToBrowser(command)) as { content: string };
